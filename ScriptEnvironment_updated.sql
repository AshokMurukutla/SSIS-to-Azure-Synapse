/****** Object:  StoredProcedure [dbo].[usp_SSIS_ScriptEnvironment]    Script Date: 23-08-2022 12:33:59 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[usp_SSIS_ScriptEnvironment]
          @folder sysname
        , @env sysname
AS
        SET NOCOUNT ON;
        ...
        */        WHILE EXISTS (SELECT TOP 1 1 FROM #env_var)
        ...
        SET @sql += @tab + 'IF NOT EXISTS (SELECT 1 FROM [SSISDB].[internal].[folders] WHERE name = @folder)' + @cr;
        ...
        SET @sql += @tab + @tab + 'IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback1;' + @cr;
        SET @sql += @tab + 'END' + @cr + @cr;   
        SET @sql += @tab + 'RAISERROR(''Creating Environtment: %s'', 10, 1, @env) WITH NOWAIT;' + @cr;
		SET @sql += @tab + 'IF NOT EXISTS (SELECT 1 FROM [SSISDB].[internal].[environments] env INNER JOIN [SSISDB].[internal].[folders] fol ON env.folder_id=fol.folder_id WHERE environment_name = @env and name=@folder)' + @cr;
        SET @sql += @tab + 'BEGIN' + @cr;
        SET @sql += @tab + @tab + 'EXEC @ReturnCode = [SSISDB].[catalog].[create_environment] @folder_name=@folder, @environment_name=@env'  + @cr;
        SET @sql += @tab + @tab + 'IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback1;' + @cr + @cr;
		SET @sql += @tab + 'END' + @cr + @cr;
        SET @sql += @tab + '/******************************************************' + @cr;
        ...
        /* Generate the variable creation */        SELECT [cmd] = @tab + 'RAISERROR(''Creating variable: ' + ev.name + ' ...'', 10, 1) WITH NOWAIT;' + @cr
										+ @tab + 'IF NOT EXISTS (SELECT 1 FROM [SSISDB].[internal].[environment_variables] env_var INNER JOIN [SSISDB].[internal].[environments] env ON  env_var.environment_id=env.environment_id INNER JOIN [SSISDB].[internal].[folders] fol ON env.folder_id=fol.folder_id where env.environment_name=N''' + @env + ''' and fol.name = N''' + @folder + ''' and env_var.name = N''' + ev.name + ''')'
                                        + @tab + 'BEGIN' + @cr
										+ @tab + @tab + 'EXEC @ReturnCode = [SSISDB].[catalog].[create_environment_variable]' + @cr
                                        + @tab + @tab + @tab +'@variable_name=N''' + ev.name + '''' + @cr
                                        + @tab + @tab + @tab +', @sensitive=' + CONVERT(varchar(2), ev.sensitive) + @cr
                                        + @tab + @tab + @tab +', @description=N''' + ev.[description] + '''' + @cr
                                        + @tab + @tab + @tab +', @environment_name=@env' + @cr
                                        + @tab + @tab + @tab +', @folder_name=@folder' + @cr
                                        + @tab + @tab + @tab +', @value=@' + ev.name + @cr
                                        + @tab + @tab + @tab +', @data_type=N''' + ev.type + '''' + @cr
                                        + @tab + @tab + 'IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback1;' + @cr
										+ @tab + 'END' + @cr + @cr
                                , [name] = ev.name
        INTO #cmd
        FROM [SSISDB].[catalog].[folders] f        
